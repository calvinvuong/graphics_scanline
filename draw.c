...
void draw_polygons( struct matrix *polygons, screen s, color c ) {
  ...
  for (point=0; point < polygons->lastcol-2; point+=3) {
    ...
    if ( normal[2] > 0 ) { // triangle should be drawn
      double max = max_vertex(polygons->m[1][point], polygons->m[1][point+1], polygons->m[1][point+2]);
      double min = min_vertex(polygons->m[1][point], polygons->m[1][point+1], polygons->m[1][point+2]);
      ...
      for ( j = point; j < point+3; j++ ) {
	if ( polygons->m[1][j] == max && top_filled == 0) {
	  top_point_index = j;
	  top_filled += 1;
	}
	else if ( polygons->m[1][j] == min && bottom_filled == 0 ) {
      ...
      float x1 = polygons->m[0][bottom_point_index];
      float y = polygons->m[1][bottom_point_index];
      float dy = 1;
      float dx0;
	...
	dx1 = (polygons->m[0][middle_point_index] - polygons->m[0][bottom_point_index]) / (polygons->m[1][middle_point_index] - polygons->m[1][bottom_point_index]);
      ...
      while ( y <= polygons->m[1][middle_point_index] ) {
	c.green = (point * 13) % 256;
	c.blue = (point * 17) % 256;
	c.red = (point * 23) % 256;
	draw_line((int)x0, (int)y, (int)x1, (int)y, s, c);
	x0 += dx0;
	x1 += dx1;
	y += dy;
      }
	...
	dx1 = (polygons->m[0][top_point_index] - polygons->m[0][middle_point_index]) / (polygons->m[1][top_point_index] - polygons->m[1][middle_point_index]);
      x1 = polygons->m[0][middle_point_index];
